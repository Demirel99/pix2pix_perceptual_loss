...
class VGGPerceptualLoss(nn.Module):
    """
    Defines the VGG-based perceptual loss.
    
    Uses VGG19 pretrained on ImageNet to extract features from multiple layers.
    The loss is the L1 distance between feature maps of the generated and target images.
    """
    def __init__(self, device):
        super(VGGPerceptualLoss, self).__init__()
        
        # Load pre-trained VGG19 and select layers
        ...
        # Define the feature layers to be used
        # These are the layers after ReLU activations, which are commonly used.
        self.feature_layers = [3, 8, 17, 26, 35] # conv1_2, conv2_2, conv3_2, conv4_2, conv5_2
        self.vgg_layers = nn.ModuleList([vgg[i] for i in range(max(self.feature_layers) + 1)])
    ...
    def _normalize(self, x):
        # The input x is in the range [-1, 1] from the generator's Tanh activation
    ...
    def forward(self, generated, target):
        ...
        Parameters:
            generated (tensor) -- the generated image from the generator (fake_B)
            target (tensor)    -- the ground truth image (real_B)
        """
        generated_norm = self._normalize(generated)
        target_norm = self._normalize(target)
        
        loss = 0.0
        
        # Pass images through VGG layers and compute loss at specified feature layers
        current_layer_idx = 0
